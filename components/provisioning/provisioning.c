...
provisioning_start (const char *system_name, provisioning_state_cb_t callback, void *user_data)
{
    ...
    if (s_callback != NULL)
        {
            s_callback (s_logic.state, s_user_data);
        }
    if (provisioned || s_started)
        {
            return ESP_OK;
        }
    ...
    esp_err_t err = network_prov_mgr_start_provisioning (NETWORK_PROV_SECURITY_1, PROVISIONING_POP,
                                                         service_name, NULL);
    if (err != ESP_OK)
        {
            network_prov_mgr_deinit ();
